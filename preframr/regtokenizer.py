...
class RegTokenizer:
    ...
    def train_tokenizer(self, dfs):
        ...
        self.logger.info(
            f"starting training {self.args.tokenizer} tokenizer with {frame_tokens} frame tokens"
        )
        ...
        p = ctx.Process(
            target=train_worker,
            args=(self.args.tokenizer, self.args.tkvocab, self.args.tkmodel, uni_files),
        )
        p.start()
        p.join()
