...
def remove_voice_reg(orig_df, reg_widths):
    ...
    if voice_regs:
        ...
        df.loc[m, "v"] = (np.right_shift(df[m]["sval"], df["fn"] * 2) & 2**2 - 1) - 1
        ...
        for i, row in enumerate(f_df.itertuples()):
            if row.reg < 0:
                if row.reg == FRAME_REG:
                    f_sid_writes.append((row.reg, row.val, row.diff, row.description))
                elif row.reg == DELAY_REG:
                    for _i in range(row.val - 1):
                        delay_sid_writes = [(FRAME_REG, 0, frame_diff, row.description)]
            ...
            f_sid_writes.append((row.reg, last_val[row.reg], row.diff, row.description))
...
class RegLogParser:
    ...
    def _get_vmeta(self, df):
        ...
        for xdf, reg in ((freq_df, 0), (ctrl_df, 4)):
            ...
            xdf = xdf[["f", "v", c]].reset_index(drop=True)
            xdf = xdf.ffill().fillna(0).drop_duplicates(["f", "v", c], keep="last")
            df = df.merge(xdf, how="left", on=["f", "v"])
    ...
    def _norm_pr_order(self, orig_df):
        ...
        df = df.sort_values(["f"] + cols + ["v", "reg", "n"])
    ...
    def _add_voice_reg(self, orig_df, zero_voice_reg=True):
        ...
        if zero_voice_reg:
            ...
            for v in range(VOICES):
                m = (df["v"] == v) & (df["reg"] == 0) & (df["op"] == SET_OP)
                df.loc[m, f"{v}freqmeta"] = np.right_shift(
                    df[m]["val"], self.freq_mapper.bits - META_FREQ_BITS
                )
                df.loc[m, f"{v}ctrlmeta"] = df[m]["val"] & 0b11110000
            df = df.ffill().fillna(0)
            m = df["reg"] == VOICE_REG
            df.loc[m, "v"] = df[m]["val"]
            for v in range(VOICES):
                m = (df["reg"] == VOICE_REG) & (df["v"] == v)
                df.loc[m, "val"] = df[m][f"{v}freqmeta"] + df[m][f"{v}ctrlmeta"]

        df.loc[df["reg"] == FRAME_REG, "val"] = df[df["reg"] == FRAME_REG]["svt"]
        df = df[orig_df.columns].reset_index(drop=True)
        return df
