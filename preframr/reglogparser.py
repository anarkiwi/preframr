...
class RegLogParser:
    ...
    def _add_voice_meta(self, orig_df):
        df = self._norm_df(orig_df)
        for v in range(VOICES):
            col = f"{v}meta"
            m = (df["v"] == v) & (df["reg"] == 0) & (df["op"] == SET_OP)
            df.loc[m, col] = np.left_shift(
                np.right_shift(df[m]["val"], self.freq_mapper.bits - 4), v * 4
            )
        df = df.ffill().fillna(0)
        df["meta"] = 0
        for v in range(VOICES):
            ...
            df["meta"] += df[col]
        m = df["reg"] == FRAME_REG
        df.loc[m, "meta"] = np.left_shift(df[m]["meta"], 7)
        df.loc[m, "val"] += df[m]["meta"]
        df = df[orig_df.columns].reset_index(drop=True)
        return df
