...
def write_samples(
    ...
    if sid is None:
        sid = default_sid()
    if descriptions is None or not descriptions:
        descriptions = ["prompt"]
    ...
    with AsidProxy(sid=sid, asid=asid, cents=cents, sysex_delay=sysex_delay) as proxy:
        ...
        if "description" not in df.columns:
            df["description"] = 0
        last_row = None
        pbar = tqdm(df.itertuples(), total=len(df), desc=descriptions[0])
        for row in pbar:
            if last_row is not None and row.description != last_row.description:
                pbar.set_description(descriptions[row.description])
            if row.reg < 0:
